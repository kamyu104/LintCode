...
Up to date (2015-05-30), there are `226` problems on [LintCode Online Judge](https://lintcode.com/).
...
|401|[Kth Smallest Number in Sorted Matrix](http://lintcode.com/en/problem/kth-smallest-number-in-sorted-matrix/)| [C++](./C++/kth-smallest-number-in-sorted-matrix.cpp)| _O(klog(min(m, k)) + klog(min(n, k)))_ | _O(min(m, k) + min(n, k))_ | Medium | | BST, Heap |
